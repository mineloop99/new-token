...
contract SpendAni is AccessControl {
    ...
    function addItemShop(string memory _itemName, uint256 _amount) public {
        ...
        shopName.push(_itemName);
        shopInfo[shopName[shopName.length - 1]] = _amount;
    }
    ...
    function removeItemShop(string memory _itemName) public {
        ...
        for (uint256 i = 0; i < shopName.length; i++) {
            if (keccak256(abi.encodePacked(shopName[i])) == _temp) {
                delete shopName[i];
                return;
            }
        }
    ...
    {
        uint256[] memory _temp = new uint256[](shopName.length);
        for (uint256 i = 0; i < shopName.length; i++) {
            _temp[i] = shopInfo[shopName[i]];
        }
        return (shopName, _temp);
    }
}
